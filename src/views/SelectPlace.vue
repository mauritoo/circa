...
const locationStore = useLocationStore();
  ...
  const fetchLocations = async () => {
    const response : MyLocation[] = await locationStore.fetchLocations(search.value)
    if (response && response.length > 0)
      locations.value = response.map(
        loc => ({value: loc, title: `${loc.name} ${loc.state ? ', '+loc.state : ''} ${loc.country ? ', '+loc.country: ''}`}))
  }
  ...
  watch(select, async () => {
    if (select.value) {
      ...
      await sunriseStore.fetchTodayInfo(select.value.name, Number(select.value.lat), Number(select.value.lon))
  ...
  const noDataText = computed(() => search.value && !loading.value? 'No se encontraron resultados' : '')
  lastLocations.value = [
    {
      name: 'Hurlingham',
      state: 'Buenos Aires',
...
<template>
  <div class='select px-6 mt-5'>
    ...
    <div v-if="lastLocations.length>0">
      <v-card v-for="location in lastLocations" 
        :key="location.lon"
